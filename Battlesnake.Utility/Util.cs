...
namespace Battlesnake.Utility
{
    public class Util
    {
        ...
        public static void LikelihoodOfSuperiority(int wins, int draws, int losses)
        {
            double games = wins + draws + losses;
            Console.WriteLine($"Number of games: {games}");
            double winningFraction = (wins + 0.5d * draws) / games;
            Console.WriteLine($"Winning fraction: {winningFraction}");
            double eloDiffernece = -Math.Log(1.0d / winningFraction - 1.0d) * 400.0d / Math.Log(10.0d);
